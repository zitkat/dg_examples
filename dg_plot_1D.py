...
def load_and_plot_fun(folder, filename, t0, t1, tn,
    ...
    a1, a2 = animate_1D_DG_sol(lmesh, t0, t1, u, tn=tn, ic=ic_fun, exact=exact,
    ...
    if compare:
        ...
        ww_s, xx = reconstruct_legendre_dofs(coors, None, u_start)
        ww_e, _ = reconstruct_legendre_dofs(coors, None, u_end)
        XN = xx[-1]
        X1 = xx[0]
        plt.plot([X1, XN], [.5, .5], 'grey', alpha=.6)
        plt.plot([X1, XN], [0, 0], 'grey', alpha=.6)
        plt.plot(xx, ww_s[:, 0], label="p(0, x)")
        plt.plot(xx, ww_e[:, 0], label="p(1, x)")
        plt.legend()
    plt.show()
